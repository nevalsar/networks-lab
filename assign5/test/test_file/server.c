...
int main(void) {
    ...
    inet_ntop(p->ai_family, &(p->ai_addr), s, INET6_ADDRSTRLEN);
    ...
    while (1) {  // main accept() loop
        ...
        if (!fork()) {  // this is the child process
            ...
            FILE *inputFile = fopen("random.txt", "rb");
            if (inputFile == NULL) {
            ...
            // TODO(routeaccess): Check for errors here
            char sendBuffer[10];
            int bytesRead = fread(sendBuffer, sizeof(sendBuffer), 1, inputFile);

            while (!feof(inputFile)) {
                // TODO(routeaccess): check for errors here
                send(new_fd, sendBuffer, bytesRead, 0);
